...
def exon(count,mRNA_id,mRNA_st,strand,model_no,mRNA_en,chr_id,no):
	exons = {}
	exon_no = 0
...
def call_CDS(count,mRNA_id,gene_model,mRNA_st,strand,model_no, exons, mRNA_en, chr_id,no):
	...
	for line in open(file,'r'):
		if line.startswith('>'):
			...
			if (cds_strand == strand) & (chr_id == tokens[0][1:]):
				...
				if strand == '+':
					if len(exons)==1:
						...
						print five_prime_UTR_st, five_prime_UTR_en, introns, start
						ID = 'ID='+mRNA_id+'.'+str('5_prime_UTR')+';'+'Parent='+mRNA_id+'.'+str(no)+';'
						lin = token[0][1:]+'\t'+'TAU'+'\t'+'five_prime_UTR'+'\t'+str(five_prime_UTR_st)+'\t'+str(five_prime_UTR_en)+'\t'+'.'+'\t'+strand+'\t'+'.'+'\t'+ID
						o.write(lin+'\n')
						CDS_st = start + introns
						if exons[j][1] - introns >= end:
							lin = token[0][1:]+'\t'+'TAU'+'\t'+'CDS'+'\t'+str((end+introns)+mRNA_st)+'\t'+str(exons[j][1]+ mRNA_st)+'\t'+'.'+'\t'+strand+'\t'+str(frame)+'\t'+ID
							o.write(lin+'\n')
					...
					three_prime_UTR_en = exons[len(exons)][1] + mRNA_st + 1
				...
				if strand == '-':
					if len(exons)==1:
						...
						if exons[len(exons)][1] - exons[j][0] - introns >= end:
							lin = token[0][1:]+'\t'+'TAU'+'\t'+'CDS'+'\t'+str(exons[len(exons)][1] - (end+introns)+mRNA_st)+'\t'+str(exons[j][1]+ mRNA_st)+'\t'+'.'+'\t'+strand+'\t'+str(frame)+'\t'+ID
							o.write(lin+'\n')
							CDS_en = exons[len(exons)][1] - (end+introns)
...
def mRNA(count,gene_id,gene_model,mRNA_st,strand,model_no):
	...
	for line in open(file,'r'):
		if line.startswith('>'):
			...
			exons = exon(count,mRNA_id,mRNA_st,strand,model_no,end,tokens[0][1:],no)
			### call CDS/UTRs
			call_CDS(count,mRNA_id,gene_model,mRNA_st,strand,model_no,exons,end,tokens[0][1:],no)
	...
	for line in open(gff,'r'):
		...
		if (token[2] == 'mRNA'):
			...
			if new_transcript == True:
				...
				### run TAU
				os.system('nice -n 19 perl /u/vgupta/01_genome_annotation/tools/TAU/TAU.pl -A temp.fa -G temp.gff -O temp'+str(count) +' ')	
				file = './temp'+str(count)+'/temp'+str(count)+'.cdna.fa'
...
if __name__ == "__main__":
	...
	for line in open(gff,'r'):
		...
		if token[2] == "gene":
			if token[1] == "CUFFLINKS":
				...
				id = line.split('ID=')[1].split(';')[0]
				hash_gff_gene[id]=line
				if token[1]=="Stegodyphus_mimosarum.v1.final.fa": ### adding exception for velvet de novo
					hash_gff_mRNA[id] = line.replace('gene','mRNA')
					mRNA_id[id] = id
					hash_gff_exon[mRNA_id[id]] = line.replace('gene','exon')
	...
	#file_empty(gff)
	#gff = 'sorted.gff3'
	o = open('20121116_TAU_genemodel.noHomology.gff3','w')
