...
def print_homoeo_pair(seq):
    os.system('rm -rf 04_ToTr_CDS')
    os.system('mkdir -p 04_ToTr_CDS') 
    count = 0
    for line in open(infile, 'r'):
        ...
        if len(tokens) == 2:
            if tokens[0] in seq and tokens[1] in seq:
                count += 1
                outfile = '/array/users/vgupta/14_cloverAnalysis/08_regele/04_ToTr_CDS/'+str(str(count).zfill(4))+'_homoeolog'+'.fa'  
                o = open(outfile,'w')
                o.write('>'+tokens[0]+'\n')
