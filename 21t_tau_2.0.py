		...
		if line.startswith('>'):
			...
			if strand == '-':
				if len(exons)==1:
					...
					print exons[len(exons)][1] - (end+introns) - 1+ mRNA_st,exons[len(exons)][1] - (end+introns)+mRNA_st
				...
				print three_prime_UTR_st,three_prime_UTR_en
				ID = 'ID='+mRNA_id+'.'+str('3_prime_UTR')+';'+'Parent='+mRNA_id+';'
...
if __name__ == "__main__":
	...
	#	os.system("cat "+gff+ "| perl -pe '$_=~s/\+/./'|perl -pe '$_=~s/\-/./' > sorted.gff3")
	#file_empty(infile)
	#file_empty(gff)
