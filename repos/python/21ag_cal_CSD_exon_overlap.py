...
def logfile(infile):
    ...
    o.write("Program used: \t\t%s" % "21ag_cal_CSD_exon_overlap.py")
...
def options(argv):
    ...
    try:
        ...
        print 'python 21ag_cal_CSD_exon_overlap.py -i <inputfile>'
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print 'python 21ag_cal_CSD_exon_overlap.py -i <inputfile>'
    ...
    for line in open(file,'r'):
        ...
        if token[2] == 'gene': ##chr5    GeneMark.hmm    gene    17668036    17668216    .    .    .    ID=100131_g;Name=100131_g
            ...
            if first_line == False:
                ...
                o.write(str(round(100*float(cdsSize)/sum(exon),2))+'\n')
            ...
            first_line = False
        if token[2] == 'exon':
            exon.append(int(token[4]) - int(token[3]))
        if token[2] == 'CDS': #chr5    TAU    CDS    17668104    17668214    .    +    0    ID=100131_g.1.CDS.1;Parent=100131_g.1;
            cds.append(int(token[4]) - int(token[3]))
    cdsSize = sum(cds)
    o.write(str(100*float(cdsSize)/sum(exon))+'\n')
    o.close()        
