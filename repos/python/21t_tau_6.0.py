    ...
    for line in open(file,'r'):
        ...
        if line.startswith('>'):
            ...
            if Longheader == line:            
                ...
                lin = token[0][1:]+'\t'+gene_model+'\t'+'mRNA'+'\t'+str(mRNA_st)+'\t'+str(mRNA_en)+'\t'+'.'+'\t'+tokens[4].strip()+'\t'+'.'+'\t'+ID
                        
                o.write(lin+'\n')
                mRNA_st = start -1
                print file, model_no,"flag", lin
                ### call exons from TAU gff file
                exons = exon(count,mRNA_id,mRNA_st,mRNA_en,strand,model_no,tokens[0][1:],no)
                ### call CDS/UTRs
                call_CDS(count,mRNA_id,gene_model,mRNA_st,mRNA_en,strand,model_no,exons,tokens[0][1:],no)
