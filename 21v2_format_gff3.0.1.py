...
def options(argv):
    ...
    oMCLfamily = ''
    try:
       opts, args = getopt.getopt(argv,"hi:g:c:p:b:r:o:f:s:",["ifile=","gfile=","cfile=","pfile=", "bfile=","rfile=","orthoMCL=","oMCLfamily=","blast_search=",])
    except getopt.GetoptError:
       print 'python 21v_format_gff3.py -i <inputfile> -g <gfile> -c <cfile> -p <pfile> -b  <homology> -r <rfile> -o <orthoMCL> -f <oMCLfamily> -s <blast_search> ' 
       sys.exit(2)
    for opt, arg in opts:
    	if opt == '-h':
    		print 'python 21v_format_gff3.py -i <inputfile> -g <gfile> -c <cfile> -p <pfile> -b  <homology> -r <rfile> -o <orthoMCL> -f <oMCLfamily> -s <blast_search>'
    ...
    return inputfile, genomefile, cdsfile, proteinfile, homologyfile, exon_repeat, oMCLfile, oMCLfamily, blastfile
	...
	for line in open(inf,'r'):
		...
		if len(token) > 3:
			if token[2] == "mRNA":
				...
				print str(count)+'\t'+source+'\t'+ID+'\t'+token[0]+'\t'+token[3]+'\t'+token[4]+'\t'+token[6]+'\t'+cdnaSeq+'\t'+str(len(cdnaSeq))+'\t'+cdsSeq+'\t'+str(len(cdsSeq))+'\t'+proteinSeq+'\t'+str(len(proteinSeq))+'\t'+blast_hit+'\t' + exon + '\t' + str(round(float(len(cdsSeq))/len(cdnaSeq),2)) + '\t' + oMCL + '\t' +ofamily+'\t'+ blast_hit_UniProt + '\t' + proteins +'\t' + gene_name


if __name__ == "__main__":
    
    inf, gfile, cfile, pfile, hfile, exon_repeat, oMCLfile, oMCLfamily, blastfile = options(sys.argv[1:])
    ...
    format(inf, cDNA, cds, protein, homology, exons, orthoMCL, orthofamily, blast)
