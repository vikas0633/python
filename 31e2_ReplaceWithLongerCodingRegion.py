    ...
    for line in open(last_gff3,'r'):
        ...
        if len(line) > 0 and not line.startswith('#'):
            ...
            if re.search('ID=',line):
                ...
                if obj.types()=='mRNA':
                    if obj.seqids() == chromosome and (start_min <= int(obj.starts()) <  start_max) :
                        ID = str(obj)
                        CDS[ID] = 0
                        exons[ID] = []
                        hash_flag = True
                        ...
                        hash_flag = False
                if obj.types()=='exon' and obj.seqids() == chromosome and hash_flag == True:
                    exons[ID].append( (int(obj.starts()),int(obj.ends())) )
                if obj.types()=='CDS' and obj.seqids() == chromosome and hash_flag == True:
                    if ID in CDS:
                        CDS[ID] += (int(obj.ends()) - int(obj.starts()) + 1)                  
    
    return exons, CDS

def find_gene_overlaps(evidences, exons_transcript, CDS_transcript, exons, CDS, chromosome, start_min, start_max):
    count = 0
    for line in open(last_gff3,'r'):
        line = line.strip()
        if len(line) > 0 and not line.startswith('#'):
            ...
            obj = classGene.GFF3(line)
            if obj.types()=='mRNA' and obj.seqids()==chromosome and (start_min <= int(obj.starts()) <  start_max):
                ID = str(obj)
...
def hash_evidences(chromosome, exons, CDS, start_min, start_max):
    ...
    find_gene_overlaps(evidences, exons_transcript, CDS_transcript, exons, CDS, chromosome, start_min, start_max)



def parse(chromosome, start_min, start_max):
    
    exons, CDS = hash_last_gff3_CDS(chromosome, start_min, start_max)
    
    hash_evidences(chromosome, exons, CDS, start_min, start_max)
...
if __name__ == "__main__":
    ...
    for chromosome in sorted(chroHash):
        for i in range(0,int(chroHash[chromosome]), 10000000):
            parse(chromosome, i, i+10000000)
